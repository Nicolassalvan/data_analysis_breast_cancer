...
```{r}
res.pca <- PCA(data[2:27], scale.unit = TRUE, graph = TRUE, quali.sup = 1)
```

On n'arrive pas à distinguer les individus sur le plan principal. L'ACP n'est pas adaptée pour ce type de données. 
...
```{r}
plot(res.pca, choix = "var", habillage = 1, cex = 0.8, col.var = "black", select = "contrib 5")
```
